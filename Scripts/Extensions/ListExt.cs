...
namespace KoganeUnityLib.ListEx
{
    /// <summary>
    ...
    public static class ListExt
    {
        ...
        {
            for (var i = 0; i < collectionList.Length; i++) list.AddRange(collectionList[i]);
        }
        ...
        ///     複数のキーを使用して要素を並べ替えます
        /// </summary>
        public static void Sort<TSource, TResult1, TResult2>(this List<TSource> self,
                                                             Func<TSource, TResult1> selector1,
                                                             Func<TSource, TResult2> selector2)
            where TResult1 : IComparable where TResult2 : IComparable
        {
            self.Sort((x, y) =>
            {
                int result = selector1(x).CompareTo(selector1(y));
            ...
            where TResult1 : IComparable where TResult2 : IComparable
        {
            self.Sort((x, y) =>
            {
                int result = selector1(y).CompareTo(selector1(x));
                return result != 0 ? result : selector2(x).CompareTo(selector2(y));
            });
        }
        ...
        {
            while (count <= self.Count) self.RemoveAt(self.Count - 1);
        }
        ...
        {
            for (int i = startIndex; i < endIndex; i++) self.Add(value);
        }
